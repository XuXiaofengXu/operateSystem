...
extern char* systemStartAddr;  //外部变量，系统起始地址
...
void initRootDir()
{
    ...
    int startBlock;
    if(systemStartAddr[bitMapSize] !='1' ){  //如果第101块盘块未被使用，说明根目录未分配空间。
        //分配一个盘块空间给rootDirTable
        startBlock = getBlock(1);
        if(startBlock == -1)
            return;
    }else{
        startBlock=bitMapSize; //如果第101块盘块已经被使用，说明已经分配空间。（假设盘块数量为100）
    }
    ...
    //初始化初始绝对路径
    path[0]='\\';   //双斜杠，一个斜杠是为了转义，实际的作用效果就是一个斜杠
    path[1]='\0';   //结束符
}
...
int changeDir(char dirName[])
{
    ...
    //修改全局绝对路径
    if(strcmp(dirName, "..") == 0)  //如果是返回上一级菜单，那么就把绝对路径删除至最后一个斜杠的地方
    {
        //回退绝对路径
        int len = strlen(path);
...
int creatFCB(int fcbBlockNum, int fileBlockNum, int fileSize)
{
    ...
    currentFCB->reader_num=0;//读者的人数
    currentFCB->sem_write=1;//写信号量
    currentFCB->sem_mutex=1;//读写互斥信号量
    return 0;
}
...
int addDirUnit(dirTable* myDirTable, char fileName[], int type, int FCBBlockNum)
{
    ...
    //构建新目录项
    dirUnit* newDirUnit = &myDirTable->dirs[dirUnitAmount];  //新建一个指针指向需要新添加目录项的位置
...
int doRead(FCB* myFCB, int length)
{
    ...
    char* data = (char*)getBlockAddr(myFCB->blockNum);  //获取数据的起始块所在的地址
    //在不超出数据长度下，读取指定长度的数据
